...
### Tests
- `test/network_test.rb` (13): genesis hashes asserted literally rather than round-tripped through the table that defines them; the three-state alignment, each state separately; `canonical` returning nil rather than a default for an unknown name.
...
- `test/gemspec_test.rb` (5): the packaging invariant, no directory entries, version/constant agreement, and no Rails runtime dependency.
- `test/network_guard_js_test.rb` (17): the classifier under node, including the rule-ordering regression (`/^unexpected/` must not claim a simulation failure) and the wrapper's transparency — original error re-thrown, success passed through, a throwing hint callback unable to replace the real error.
